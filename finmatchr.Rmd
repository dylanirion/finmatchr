...
update_results <- observeEvent( react$update, {
  cat("old_from: ", payload$old_from, "\n")
  cat("old_to: ", payload$old_to, "\n")
  graph <- payload$graph
  n <- NULL
  e <- NULL
  #update the graph
...
db_stats <- observeEvent( input$refresh, {
  withProgress( message = "Calculating Statistics...", value = 0, {
    ...
    for( i in 1:length(unique(dates$year)) ) {
      ...
      for( j in 1:length(unique(tmp$month)) ) {
        date_list[[i]]$children[[j]] <- list()
        date_list[[i]]$children[[j]]$text <- as.character(unique(tmp1$month)[j])
        date_list[[i]]$children[[j]]$data <- list( query = paste0( "img1.date.year = ", unique(dates$year)[i], " AND img1.date.month = ", unique(dates$n_month)[i] ) )
