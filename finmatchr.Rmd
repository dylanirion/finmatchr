...
tags$script( paste0( "$(document).on('keydown', function (e) {Shiny.setInputValue('keydown', e.which, {priority: 'event'});});",
                     ...
                     "  Shiny.setInputValue('from_img_idx', id);",
                     "});",
                     "Shiny.addCustomMessageHandler('setToImgIDX', function(id) { ",
                     "  Shiny.setInputValue('to_img_idx', id);",
                     "});" ) )
...
update_edge <- function( from_uuid, to_uuid, label, basis, reviewed ) {
  ...
  query <- paste0( "MATCH (img1:Image)-[:IS_SAME*2..4]->(img2:Image) ",
                   ...
                   "AND img1.uuid IN ['", from_uuid, "', '",to_uuid, "'] ",
  ...
  query <- paste0( "MATCH (img1:Image)-[:IS_SAME*2..4 {reviewed: true}]->(img2:Image) ",
                   "WHERE id(img1) <> id(img2) AND NOT (img1)<-[:IS_SAME {reviewed: true}]->(img2) ",
                   "AND img1.uuid IN ['", from_uuid, "', '",to_uuid, "'] ",
                   "AND COALESCE(img1.unusable, false) <> true AND COALESCE(img2.unusable, false) <> true ",
...
watch_graph_click <- observeEvent( input$edge_id, {
  if( !input$edge_label %in% c( 'HAS_ENCOUNTER', 'HAS_IMAGE' ) ) {
    payload$from_meta <- payload$graph$nodes %>% filter( id == input$edge_from_id ) %>% select( -id, -label ) %>% unnest( cols = c(data) )
    payload$to_meta <- payload$graph$nodes %>% filter( id == input$edge_to_id )  %>% select( -id, -label ) %>% unnest( cols = c(data) )
...
from <- observeEvent( payload$from_meta, {
...
draw_from <- eventReactive( input$from_img_idx, {
  img <- payload$from_meta %>% slice( input$from_img_idx )
  output$from_id <- renderText({ paste0( img %>% .$uuid, " [", input$from_img_idx, "/", nrow(payload$from_meta), "]" ) })
...
from <- observeEvent( payload$to_meta, {
...
draw_to <- eventReactive( input$to_img_idx, {
  img <- payload$to_meta %>% slice( input$to_img_idx )
  output$to_id <- renderText({ paste0( img %>% .$uuid, " [", input$to_img_idx, "/", nrow(payload$to_meta), "]" ) })
...
keydown <- observeEvent( input$keydown, {
  if( ifelse( is.null( input$graphSelected ), F, input$graphSelected ) ) {
    if( input$keydown == 37 | input$keydown == 39 ) { #L/R arrows
      ...
      if( input$keydown == 37 ) {  # left arrow
      ...
      } else if( input$keydown == 39 ) {  # right arrow
        if( length(unreviewed_i) > 0 ) {
    ...
    } else if( input$keydown == 88 | input$keydown == 67 ) {
      nodes <- payload$from_meta
      j <- input$from_img_idx
      if( input$keydown == 88 ) {  # x
        j <- ifelse(  j == 1, length( nodes$id ), j - 1 )
      } else if( input$keydown == 67 ) {  # c
        j <- ifelse( j == length( nodes$id ), 1, j + 1 )
      }
      session$sendCustomMessage( "setFromImgIDX", j )
    } else if( input$keydown == 188 | input$keydown == 190 ) {
      nodes <- payload$to_meta
      j <- input$to_img_idx
      if( input$keydown == 188 ) {  # <
        j <- ifelse(  j == 1, length( nodes$id ), j - 1 )
      } else if( input$keydown == 190 ) {  # >
        j <- ifelse( j == length( nodes$id ), 1, j + 1 )
      }
