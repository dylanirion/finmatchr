...
update_edge <- function( from_uuid, to_uuid, label, basis, reviewed ) {
  ...
  query <- paste0( "MATCH (img1:Image)<-[:IS_SAME*2..2]->(img2:Image) ",
  ...
  query <- paste0( "MATCH (img1:Image)<-[:IS_SAME*2..2 {reviewed: true}]->(img2:Image) ",
                   "WHERE id(img1) <> id(img2) AND NOT (img1)<-[r:IS_SAME {reviewed: true}]->(img2) ",
                   "AND img1.uuid IN ['", from_uuid, "', '",to_uuid, "'] ",
                   "AND COALESCE(img1.unusable, false) <> true AND COALESCE(img2.unusable, false) <> true ",
...
# triggers "run" with increment
#@todo: how to improve speed of algo.cover?
