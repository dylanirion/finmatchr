...
#some styling maps
color.map <- c( IS_SAME = '#5CB85CFF', UNSURE = '#F0AD4EFF', NOT_SAME = '#D9534F50', HAS_ENCOUNTER = '#FFC107FF', HAS_IMAGE = '#FFC107FF', VIRTUAL = '#F2F2F2FF', Event = '#440154FF', Encounter = '#21908CFF', Image = '#FDE725FF' )
...
react <- reactiveValues( run = NULL, style = NULL, draw = NULL, update = NULL, copy = NULL, valid_img_count = NULL, unreviewed_img_count = NULL, unreviewed_folder_count = NULL, unreviewed_n_folders = NULL, date_list = NULL )
...
update_edge <- function( from_uuids, to_uuids, label, basis, reviewed ) {
  ...
  query <- paste0( "MATCH (from:Image)<-[:NOT_SAME]->(int:Image)<-[:IS_SAME*2..2]->(to:Image) ",
                   ...
                   "AND COALESCE(to.unusable, false) <> true ",
                   "MERGE (from)<-[:NOT_SAME {basis: 'propagated'}]->(to) ")
  cat( "QUERY: ", query, "\n" )
  query %>% call_neo4j( con )
...
query_date_action <- observeEvent( input$date_tree_selected, {
  ...
  if( req(length(input$date_tree_selected) > 0 ) ) {
    payload$query <- paste0( "MATCH (img1:Image) WHERE ", input$date_tree_selected[[1]]$data$query, " ",
                             "WITH img1 ",
                             "OPTIONAL MATCH (img1)<-[rel:IS_SAME]->(img2:Image) WHERE rel.basis IN ['duplicate', 'cropped'] ",
                             "WITH COLLECT(id(img1)) + COLLECT(id(img2)) AS ids, COLLECT(img1) + COLLECT(img2) AS imgs, COLLECT(rel) AS r ",
                             "UNWIND imgs AS img ",
...
run_query <- observeEvent( react$run, {
  ...
  withProgress(message = 'Querying Database...', value = 0, {
    ...
    if( length(graph) ) {
      ...
      node_cols <- tibble( unusable = NA, reviewed = NA, date = NA_character_, camera_date = NA_character_, region = NA_character_ )
      edge_cols <- tibble( id = NA_character_ , from = NA_character_, to = NA_character_, basis = NA_character_, reviewed = NA_real_, label = NA_character_ )
      # unnest nodes
...
update_results <- observeEvent( react$update, {
  ...
  if( length(graph) ) {
    ...
    if( is.null(payload$old_to) ) {
      ...
      #selecting an edge, if there is only 1, select it
      if( nrow( graph$relationships %>% filter( label %in% c( 'IS_SAME', 'UNSURE', 'NOT_SAME', 'VIRTUAL' ) ) ) == 1 | nrow( graph$relationships %>% filter( label %in% c( 'IS_SAME', 'UNSURE', 'NOT_SAME', 'VIRTUAL' ) ) ) == 0 ) {
        e <- 1
      } else {
...
watch_graph_click <- observeEvent( input$edge_id, {
  if( input$edge_id != 'EDIT SINGLE' ) {
      ...
      payload$to <- NULL
      payload$edge_meta <- NULL
      cat( "Node Select: ", input$edge_from_id, "\n" )
  }
...
db_stats <- observeEvent( input$refresh, {
  withProgress( message = "Calculating Statistics...", value = 0, {
    ...
    "CALL gds.graph.drop('valid-images') YIELD graphName" %>% call_neo4j( con, type = "row" )
    setProgress( value = 1/4 )
    ...
    setProgress( value = 3/4 )
    
    date_query <- paste0( "MATCH (i:Image) WITH DISTINCT i.date AS dates RETURN dates ORDER BY dates")
    cat( "QUERY: ", date_query, "\n" )
    dates <-  date_query %>% call_neo4j( con, type = "row" ) %>% .$dates
    dates <- dates %>%
    ...
    for( i in 1:length(unique(dates$year)) ) {
      ...
      date_list[[i]]$data <- list( query = paste0("img1.date.year = ", unique(dates$year)[i] ) )
      ...
      for( j in 1:length(unique(tmp1$month)) ) {
        ...
        date_list[[i]]$children[[j]]$data <- list( query = paste0( "img1.date.year = ", unique(dates$year)[i], " AND img1.date.month = ", unique(tmp1$n_month)[j] ) )
        ...
        for( k in 1:nrow(tmp2) ) {
          ...
          date_list[[i]]$children[[j]]$children[[k]]$text <- tmp2 %>% .$day %>% .[k]
          date_list[[i]]$children[[j]]$children[[k]]$data <- list( query = paste0( "img1.date = date('", tmp2 %>% .$value %>% .[k], "')" ) )
        }
      }
    }
